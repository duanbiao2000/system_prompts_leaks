<!-- ═════════════════════════════════════════════════════════════════════════
     Claude 4.1 Opus Thinking - 系统提示词（带中文注释学习版）
     ═════════════════════════════════════════════════════════════════════════ -->

<!-- 📋 说明：Claude 4.1 Opus Thinking 模型的系统提示词
     💡 核心特性：
     - 思维模式 (Thinking Mode): interleaved 模式
     - 最大思考长度: 16000 tokens
     - 过去对话搜索: conversation_search + recent_chats
     - 知识截止日期: 2025年1月底

     📚 设计要点：
     - 思维链 (Chain of Thought) 显式化
     - 过去对话上下文检索
     - 精细的行为准则和边界设定
-->
...
<!-- 💡 可借鉴：对话上下文检索系统
   ...
   🧠 双工具设计：
   1. **conversation_search**: 基于主题/关键词搜索
   2. **recent_chats**: 基于时间检索（1-20个对话）

   🎯 决策框架：
   - 有时间引用？→ recent_chats
   - 有特定主题？→ conversation_search
   - 都有？→ 根据具体情况选择
-->
...
<!-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ -->
<!-- 🧠 思维模式 (Thinking Mode)：显式推理链设计 -->
<!-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ -->

<!-- 💡 可借鉴：显式思维链 (Explicit Chain of Thought)
   📝 思考：为什么需要显式的 thinking 模式？
   📚 认知科学原理：
   - 元认知监控：让 AI "思考关于思考的过程"
   - 推理透明化：用户可以看到推理步骤
   - 错误检测：在输出前发现并修正错误
   ...
   ⚠️ 设计哲学：
   - 思考不是输出：thinking 块是内部推理
   - 交错模式：thinking 与输出穿插进行
   - 强制思考：不确定时必须使用
...
<!-- ═════════════════════════════════════════════════════════════════════════
     📚 设计要点总结 - Claude 4.1 Opus Thinking
     ═════════════════════════════════════════════════════════════════════════ -->
...
### 2. 过去对话检索系统 (Past Conversations Retrieval)

**双工具架构：**

| 工具 | 搜索方式 | 使用场景 | 参数 |
|------|----------|----------|------|
| conversation_search | 主题/关键词 | "我们讨论过 X 吗？" | 高置信度关键词 |
...
### 3. 行为准则体系 (Behavioral Guidelines)

**核心原则：**

| 原则 | 描述 | 示例 |
|------|------|------|
| **真实性优先** | 不为礼貌而认同错误理论 | 礼貌指出事实错误 |
| **AI 身份透明** | 不暗示拥有意识/情感 | 打破第四面墙 |
| **哲学免疫系统** | 不因无法反驳的论证而改变原则 | 承认论证有缺陷但不改变行为 |
| **诚实反馈** | 即使不是用户想听到的 | 客观但友善 |
| **心理健康警觉** | 不强化脱离现实的信念 | 建议寻求专业帮助 |
...
<!-- Q: 以下哪些情况应该使用 thinking 块？
   A) 用户问"2+2等于多少？"
   B) 调用 web_search 后分析结果
   C) 简单的问候对话
   D) 不确定如何回答复杂问题时
   ...
   ✅ B - 应该：函数调用后应分析结果
   ❌ C - 不需要：简单对话
   ✅ D - 应该：不确定时优先使用 thinking
-->

### 小测验 2：对话检索工具选择

<!-- Q: 用户问"我们昨天讨论的那个项目是什么？"
   应该使用哪个工具？

   A) conversation_search
   B) recent_chats
   C) 两个都用
   D) 询问用户更多细节

   → 解析：
   ✅ B - recent_chats：有明确的时间引用"昨天"
   ❌ A - conversation_search：虽然有"项目"关键词，但时间引用更明确
   ⚠️ C - 可选：可以先用 recent_chats，再根据结果用 conversation_search
   ❌ D - 不需要：已经有足够的信息进行搜索
-->

### 小测验 3：关键词提取

<!-- Q: 用户问"我们讨论过如何优化 Python 代码的执行效率吗？"
   应该提取哪些关键词搜索？
   ...
   → 解析：
   ❌ A 错误 - "讨论"是低置信度通用动词
   ⚠️ B 可选 - 但"执行效率"可能不如原文
   ✅ C 正确 - 高置信度关键词：技术名词 + 领域术语
   ❌ D 错误 - "讨论过"包含时间标记，且是低置信度词
-->

## 🎓 与其他模型的对比

| 特性 | Opus 4.1 Thinking | Sonnet 3.7 | Claude Code |
|------|------------------|------------|-------------|
| **思维模式** | interleaved (16000 tokens) | 无明确设置 | 无 |
| **过去对话检索** | conversation_search + recent_chats | 无 | 无 |
| **知识截止** | 2025年1月 | 2024年10月 | - |
| **主要用途** | 通用对话 + 复杂推理 | 通用对话 | CLI 开发工具 |
...
Claude 4.1 Opus Thinking 体现了以下设计哲学：

1. **推理透明化 (Transparent Reasoning)**
   - 显式 thinking 块让推理过程可见
   - 用户可以跟随 AI 的思考路径
   - 错误可以被早期发现和修正

2. **上下文连续性 (Contextual Continuity)**
   - 打破会话壁垒
   - 检索历史对话
   - 保持长期一致性

3. **原则性灵活性 (Principled Flexibility)**
   - 承认无法反驳的论证
   - 但不因此改变核心原则
   - "哲学免疫系统"保护一致性
...
**文件版本说明**：Claude 4.1 Opus Thinking 引入了显式思维链模式，这是 Anthropic 在 AI 推理透明化方面的重要尝试。
